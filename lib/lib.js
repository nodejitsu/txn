...
//
// Copyright 2011 Jason Smith, Jarrett Cruger and contributors Iris Couch
//
...
function req_couch(opts, callback) {
  ...
  return req_json(opts, function(er, resp, result) {
    if(resp && result && (resp.statusCode < 200 || resp.statusCode > 299) && result.error)
...
function req_json(opts, callback) {
  ...
  opts.strictSSL = DEFS.strictSSL;
  opts.maxSockets = DEFS.maxSockets;
  opts.headers = opts.headers || {};
  opts.headers['accept'] = opts.headers['accept'] || 'application/json';

  if (DEFS.proxy) opts.proxy = DEFS.proxy;
