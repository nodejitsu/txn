...
function Transaction (opts) {
  ...
  self.callback  = opts.callback  || DEFAULT.callback;
  self.timeout   = opts.timeout   || DEFAULT.timeout;
  self.delay     = opts.delay     || DEFAULT.delay;
  self.log       = opts.log       || DEFAULT.log;

  self.log.setLevel(DEFAULTS.log_level);

  // These can be falsy.
  self.pristine  = ('pristine'  in opts) ? opts.pristine  : DEFAULT.pristine;
...
Transaction.prototype.start = function() {
  ...
  assert.equal(typeof self.callback , 'function', 'Transaction callback required');
