...
Transaction.prototype.run = function() {
  ...
  lib.req_couch(self.req, function(er, resp, doc) {
    var is_create = !!self.create && resp.statusCode == 404 && doc.error == 'not_found';
    if(er && !is_create)
    ...
    if(is_create) {
      doc = { "_id": self.id || self.uri_to_id(self.uri) };
      self.log.debug('Creating new doc: ' + lib.JS(doc));
    }

    original.doc = lib.JDUP(doc);
    original.id  = doc._id;
    if(doc._rev)
      original.rev = doc._rev;

    if(!original.id)
      return self.emit('error', new Error('No _id: ' + lib.JS(doc)));
    if(!is_create && !original.rev)
